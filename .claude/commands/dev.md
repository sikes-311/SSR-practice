...
- **APIコントラクト**: Route Handler エンドポイント・型定義
...
|---|---|---|---|
| #1 | bff-agent | Route Handler 実装（対象シナリオに必要なエンドポイント・データ変換・DB・Downstream クライアント） | - |
| #2 | frontend-agent | フロントエンド実装（対象シナリオに必要なページ・コンポーネント・フック） | - |
| #3 | bff-test-agent | BFF ユニットテスト | #1完了後 |
...
- `src/types/{feature}.ts` への参照（型コントラクト）
...
Phase A で先行作成した E2E テストファイルを一括実行し、**全シナリオが Pass することを確認します**。
...
1. Playwright MCP でブラウザを起動し、実装されたページ（`/stocks` 等）を開く
2. 以下の観点で実際に操作して確認する:

| 観点 | 確認内容 |
|---|---|
| **初期表示** | ページロード直後の状態が仕様通りか（デフォルトソート順・表示件数など） |
| **インタラクション** | ドロップダウン・ボタン等の操作が自然に動作するか |
| **エラー表示** | API エラー時のメッセージが適切な場所・スタイルで表示されるか |
| **空状態** | データ 0 件のときに空白画面にならず適切なフィードバックがあるか |
| **レスポンシブ** | モバイル幅（375px）でレイアウトが崩れていないか |
| **アクセシビリティ** | キーボード操作・フォーカス順が自然か |

3. 問題を発見した場合は frontend-agent / bff-agent に修正を依頼する
4. 探索テストの結果（確認済み項目・発見した問題・対応状況）を 3-5 のレポートに記載する
...
|---|---|---|
| 初期表示 | ✅ 問題なし / ⚠️ {問題内容} | |
| インタラクション | ✅ 問題なし / ⚠️ {問題内容} | |
| エラー表示 | ✅ 問題なし / ⚠️ {問題内容} | |
| 空状態 | ✅ 問題なし / ⚠️ {問題内容} | |
| レスポンシブ | ✅ 問題なし / ⚠️ {問題内容} | |
| アクセシビリティ | ✅ 問題なし / ⚠️ {問題内容} | |
