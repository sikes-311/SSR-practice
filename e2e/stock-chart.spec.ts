...
// @SC-2: 株価一覧の「チャート」ボタンから株価チャートページが表示される
test('SC-2: 株価一覧の「チャート」ボタンから株価チャートページが表示される', async ({ page }) => {
  // Arrange: ログインしてトップページから株価一覧ページへ遷移する
  await loginAsTestUser(page);
  ...
  // Act: トヨタ自動車の行の「チャート」ボタンをタップする
  const toyotaCard = cards.filter({ hasText: 'トヨタ自動車' });
  await toyotaCard.locator('[data-testid="chart-button"]').click();

  // Assert: トヨタ自動車の株価チャートページが表示される
  await expect(page).toHaveURL('/stocks/TOYOTA/chart');
  await expect(page.locator('[data-testid="stock-chart-page"]')).toBeVisible();
